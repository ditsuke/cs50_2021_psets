...
unsigned int hash(const char *word)
{
    ...
    while ((c = *word++))
    {
        c = ((c >= 97 && c <= 122) ? c - 97 : (c >= 65 && c <= 90) ? c - 65
             : c);
...
{
    // Dictionary file stream
    FILE *dictstr = fopen(dictionary, "r");
    ...
    // Stores "current" word
    char cword[LENGTH + 1];
    // Stores fscanf return
    int word_read = 0;

    do
    {
        // Read a word from dict stream
...
bool unload(void)
{
    // Iterate over hash table nodes
    for (int i = 0; i < N; ++i)
    {
        node *ll = table[i];
        node *llr;

        // Transverse linked list and keep freeing memory until end of list
        while (true)
        {
            llr = ll;
            if (ll->next != NULL)
