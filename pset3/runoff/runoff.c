...
bool vote(int voter, int rank, string name)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        ...
        {
            // Set pref[vote][rank] to candidate index if name matches
...
int find_min(void)
{
    // [Hacky] Initialize min with more votes than any candidate can have
    int min = MAX_VOTERS + 1;

    // Iterate through candidates
    for (int i = 0; i < candidate_count; i++)
    {
        // For non-eliminated candidate with votes < min, min == candidate_vote
        (candidates[i].eliminated == false) ? (min > candidates[i].votes) ? (min = candidates[i].votes) : (0) : (0);
    }
    return min;
}
...
bool is_tie(int min)
{
    // [Hacky] Init base with more votes than any candidate can have
    int base = MAX_VOTERS + 1;
    for (int i = 0; i < candidate_count; i++)
    {
        // Update base with vote of first non-eliminated candidate in first iteration
